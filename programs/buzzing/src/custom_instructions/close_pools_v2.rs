...
pub fn handler(ctx: Context<ClosePoolsV2>, topic_id:u64) -> Result<()> {
    let creator = &ctx.accounts.creator;
    require!(ctx.accounts.topic.topic_id == topic_id, PredictionMarketError::InvalidTopicId);
    require_eq!(ctx.accounts.topic.creator, creator.key(), PredictionMarketError::CreatorMismatch);
...
#[derive(Accounts)]
#[instruction(topic_id: u64)]
pub struct ClosePoolsV2<'info> {
    #[account(mut)]
    pub signer: Signer<'info>,

    #[account(mut)] 
    /// CHECK: Receiver of lamports on pool close
