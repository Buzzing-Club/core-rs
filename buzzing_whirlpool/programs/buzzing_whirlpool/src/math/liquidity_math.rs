...
pub fn add_liquidity_delta(liquidity: u128, delta: i128) -> Result<u128, PredictionMarketError> {
    if delta == 0 {
        return Ok(liquidity);
    }
    if delta > 0 {
        liquidity
            ...
            .ok_or(PredictionMarketError::LiquidityOverflow)
    } else {
        liquidity
            .checked_sub(delta.unsigned_abs())
            .ok_or(PredictionMarketError::LiquidityUnderflow)
    }
}
...
pub fn convert_to_liquidity_delta(
    ...
    if liquidity_amount > i128::MAX as u128 {
        ...
        return Err(PredictionMarketError::LiquidityTooHigh);
    }
    Ok(if positive {
